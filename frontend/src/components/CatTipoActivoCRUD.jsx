...
import { Plus, Edit2, Trash2, X, AlertTriangle, Settings2 } from 'lucide-react';

const CatTipoActivoCRUD = () => {
  ...
  const fetchTiposActivo = async () => {
    try {
      const { data } = await axios.get('/cat-tipo-activo');
      setTiposActivo(data);
    } catch (error) {
  ...
  const openModal = (tipo = null) => {
    ...
    if (tipo) {
      setCurrentTipo(tipo);
      setFormData({
        tip_act_nombre: tipo.tip_act_nombre || ''
      });
    } else {
      setCurrentTipo(null);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentTipo) {
        await axios.put(`/cat-tipo-activo/${currentTipo.tip_act_id}`, formData);
      } else {
        await axios.post('/cat-tipo-activo', formData);
      }
      fetchTiposActivo();
      closeModal();
    } catch (error) {
  ...
  const handleDelete = async () => {
    setErrorMsg('');
    try {
      await axios.delete(`/cat-tipo-activo/${currentTipo.tip_act_id}`);
      fetchTiposActivo();
      closeDeleteModal();
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-white">Catálogo: Tipos de Activo</h1>
          </div>
          <button
            onClick={() => openModal()}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-800/80 text-zinc-300 border-b border-zinc-700">
                  ...
                  <th className="p-4 font-semibold">Nombre del Tipo de Activo</th>
                  <th className="p-4 font-semibold text-center w-32">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {tiposActivo.map((tipo) => (
                  <tr key={tipo.tip_act_id} className="hover:bg-zinc-800/50 transition-colors">
                    <td className="p-4 text-zinc-500">#{tipo.tip_act_id}</td>
                    <td className="p-4 text-white font-medium">{tipo.tip_act_nombre}</td>
                    <td className="p-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-zinc-800">
              <h2 className="text-xl font-bold text-white">
                {currentTipo ? 'Editar Tipo de Activo' : 'Nuevo Tipo de Activo'}
              </h2>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    name="tip_act_nombre"
                    value={formData.tip_act_nombre}
                    onChange={handleInputChange}
                    required
                    placeholder="Ej. Equipo de Navegación..."
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <p className="text-zinc-400 mb-4">
              Estás a punto de eliminar el tipo "{currentTipo?.tip_act_nombre}".
            </p>
...
export default CatTipoActivoCRUD;
